...
impl SpawnConfig {
    ...
    /// Create a spawn config for a building
    pub fn building(entity_type: EntityType, position: Vec3, player_id: u8) -> Self {
        Self {
            entity_type,
            position,
            player_id,
            unit_id: Some(0), // Buildings use unit_id 0
            scale_override: None,
            prefer_glb: false, // Buildings use primitives for now
        }
    }
    
    /// Create a spawn config for a resource
    pub fn resource(entity_type: EntityType, position: Vec3) -> Self {
        Self {
            entity_type,
            position,
            player_id: 0, // Neutral
    ...
    /// Override the scale for this entity
    pub fn with_scale(mut self, scale: f32) -> Self {
        self.scale_override = Some(scale);
        self
    }
    
    /// Set explicit unit ID
    pub fn with_unit_id(mut self, unit_id: u32) -> Self {
        self.unit_id = Some(unit_id);
        self
    }
    
    /// Force primitive shapes instead of GLB models
    pub fn force_primitive(mut self) -> Self {
        self.prefer_glb = false;
        self
    }
}
...
impl EntityType {
    ...
    /// Convert resource type to entity type
    pub fn from_resource(resource_type: ResourceType) -> Self {
...
impl EntityFactory {
    ...
    pub fn spawn(
        ...
        match &config.entity_type {
            ...
            EntityType::Building(building_type) => Self::spawn_building(commands, meshes, materials, building_type.clone(), config),
    ...
    fn spawn_unit(
        ...
        let unit_stats = Self::get_unit_stats(&unit_type);
        ...
        Self::add_unit_components(&mut entity, unit_type, unit_id, config, unit_stats, model_scale);
    ...
    fn add_unit_components(
        ...
        entity.insert((
            ...
            stats.combat, // Combat stats
            stats.vision, // Vision stats
            Selectable::default(),
...
// Implementation of statistics lookup methods
impl EntityFactory {
    fn get_unit_stats(unit_type: &UnitType) -> UnitStats {
        match unit_type {
            UnitType::WorkerAnt => UnitStats {
                health: HealthStats {
                    current: 75.0,
                    max: 75.0,
                    armor: 0.0,
                ...
                vision: Vision { sight_range: 100.0, line_of_sight: true },
                collision_radius: crate::constants::collision::WORKER_ANT_COLLISION_RADIUS,
            },
            UnitType::SoldierAnt => UnitStats {
                health: HealthStats {
                    current: 120.0,
                    max: 120.0,
                    armor: 1.0,
                ...
                vision: Vision { sight_range: 120.0, line_of_sight: true },
                collision_radius: crate::constants::collision::SOLDIER_ANT_COLLISION_RADIUS,
            },
            UnitType::HunterWasp => UnitStats {
                health: HealthStats {
                    current: 90.0,
                    max: 90.0,
                    armor: 0.0,
                ...
                vision: Vision { sight_range: 200.0, line_of_sight: true },
                collision_radius: crate::constants::collision::HUNTER_WASP_COLLISION_RADIUS,
            },
            UnitType::SpearMantis => UnitStats {
                health: HealthStats {
                    current: 110.0,
                    max: 110.0,
                    armor: 1.0,
            ...
            UnitType::ScoutAnt => UnitStats {
                ...
                vision: Vision { sight_range: 180.0, line_of_sight: true },
                collision_radius: crate::constants::collision::DEFAULT_UNIT_COLLISION_RADIUS,
            },
            UnitType::BeetleKnight => UnitStats {
                health: HealthStats {
                    current: 200.0,
                    max: 200.0,
                    armor: 3.0,
            ...
            UnitType::SpecialOps => UnitStats {
                ...
                vision: Vision { sight_range: 250.0, line_of_sight: true },
                collision_radius: 25.0, // Much larger collision for massive model
            },
            UnitType::BatteringBeetle => UnitStats {
                health: HealthStats {
                    current: 180.0,
                    max: 180.0,
                    armor: 4.0,
            ...
            // Default case for unimplemented units
            _ => UnitStats {
                health: HealthStats {
                    current: 100.0,
                    max: 100.0,
