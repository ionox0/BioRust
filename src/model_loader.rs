...
const MODEL_DEFINITIONS: &[ModelConfig] = &[
    ...
    ModelConfig::new("bee", "models/insects/bee-v1.glb#Scene0", "Classic bee model"),
    ModelConfig::new("beetle", "models/insects/black_ox_beetle_small.glb#Scene0", "Black ox beetle"),
    ModelConfig::new("ladybug", "models/insects/ladybug.glb#Scene0", "Classic ladybug"),
    ModelConfig::new("spider", "models/insects/spider_small.glb#Scene0", "Small spider"),
    ...
    ModelConfig::new("animated_spider", "models/insects/animated_spider.glb#Scene0", "Animated spider"),
    ModelConfig::new("rhino_beetle", "models/insects/a_rhino_beetle.glb#Scene0", "Rhino beetle"),
    ...
    ModelConfig::new("cairns_birdwing", "models/insects/cairns_birdwing.glb#Scene0", "Butterfly"),
...
pub fn load_models(
    ...
    info!("Loading {} insect GLB models using factory pattern...", MODEL_DEFINITIONS.len());
    ...
    load_model_handle(&mut model_assets.animated_spider, &asset_server, "animated_spider");
    load_model_handle(&mut model_assets.rhino_beetle, &asset_server, "rhino_beetle");
...
fn upgrade_units_to_glb<'a>(
    ...
    units_iter: impl Iterator<Item = (Entity, &'a RTSUnit, Mut<'a, Transform>, Option<Mut<'a, Movement>>)>,
) {
...
fn mark_buildings_as_processed<'a>(
    ...
    for (entity, building, _unit, _transform) in buildings_iter {
        info!("Keeping building as placeholder model: {:?}", building.building_type);
...
pub fn get_unit_insect_model(unit_type: &crate::components::UnitType) -> InsectModelType {
    match unit_type {
        // Worker units - ant models for thematic consistency
        crate::components::UnitType::WorkerAnt => InsectModelType::Fourmi,
        ...
        crate::components::UnitType::ScoutAnt => InsectModelType::CairnsBirdwing, // Fast butterfly
        crate::components::UnitType::SpearMantis => InsectModelType::QueenFacedBug, // Mantis
        
        // Special unit types with unique models
        crate::components::UnitType::DragonFly => InsectModelType::Meganeura, // Ancient dragonfly
        crate::components::UnitType::DefenderBug => InsectModelType::RolyPoly, // Defensive pill bug
        crate::components::UnitType::EliteSpider => InsectModelType::AnimatedSpider, // Predator
        crate::components::UnitType::SpecialOps => InsectModelType::MysteryModel, // Unknown/special
...
/// 
/// The appropriate `InsectModelType` for visual representation
pub fn get_building_insect_model(building_type: &crate::components::BuildingType) -> InsectModelType {
    match building_type {
        // Main structures - large, imposing models
        crate::components::BuildingType::Queen => InsectModelType::QueenFacedBug, // Regal mantis
        
        // Production buildings - thematic models
        crate::components::BuildingType::Nursery => InsectModelType::LadybugLowpoly, // Friendly nurturing
        crate::components::BuildingType::WarriorChamber => InsectModelType::RhinoBeetle, // Heavy training
        crate::components::BuildingType::HunterChamber => InsectModelType::AnimatedSpider, // Predator training
        
        // Resource buildings - functional models
        crate::components::BuildingType::FungalGarden => InsectModelType::Mushrooms, // Perfect match!
        crate::components::BuildingType::WoodProcessor => InsectModelType::Beetle, // Wood processing
        crate::components::BuildingType::MineralProcessor => InsectModelType::MysteryModel, // Advanced tech
...
pub fn smooth_glb_model_updates(
    mut glb_models: Query<(&mut Transform, &InsectModel, &RTSUnit), (With<UseGLBModel>, Without<Dying>)>,
    time: Res<Time>,
) {
...
fn smooth_scale_drift(transform: &mut Transform, target_scale: f32, time: &Time) {
    ...
    if scale_diff > 0.05 {
        ...
        transform.scale = transform.scale.lerp(target_scale_vec, 2.0 * time.delta_secs());
    }
}
...
fn clamp_extreme_positions(transform: &mut Transform) {
    ...
    if is_extreme {
        warn!("GLB model at extreme position {:?}, clamping", transform.translation);
        transform.translation.x = transform.translation.x.clamp(-MAX_COORDINATE, MAX_COORDINATE);
