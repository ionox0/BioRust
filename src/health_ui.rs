...
impl HealthStatus {
    pub fn from_health_ratio(ratio: f32) -> Self {
        ...
        if ratio >= HEALTH_THRESHOLD_HEALTHY {
            Self::Healthy
        } else if ratio >= HEALTH_THRESHOLD_WOUNDED {
            Self::Wounded
        } else {
            Self::Critical
        }
    ...
    pub fn color(&self) -> Color {
        use crate::constants::ui_styling::*;
        match self {
            Self::Healthy => HEALTH_BAR_HEALTHY_COLOR,
            Self::Wounded => HEALTH_BAR_WOUNDED_COLOR,
            Self::Critical => HEALTH_BAR_CRITICAL_COLOR,
        }
    }
}
