...
impl Plugin for TerrainPluginV2 {
    fn build(&self, app: &mut App) {
        app.init_resource::<TerrainChunkManager>()
...
impl CubeQuadTree {
    ...
    pub fn insert(&mut self, player_pos: Vec3) {
        self.root
            .insert(player_pos, self.min_node_size, self.distance_multiplier);
    }
...
pub fn update_visible_chunks_quadtree(
    ...
    if let Ok(camera_transform) = camera_query.get_single() {
        ...
        debug!(
            "Terrain update triggered - distance moved: {:.2}, dirty: {}",
        ...
        // Filter chunks to only include those within terrain boundary
        let filtered_chunks: Vec<ChunkKey> = new_chunks
            .into_iter()
            .filter(|chunk_key| {
                // Check if chunk is within terrain boundary
                let chunk_center_x = (chunk_key.x as f32) + (chunk_key.size as f32) * 0.5;
                let chunk_center_z = (chunk_key.z as f32) + (chunk_key.size as f32) * 0.5;
                chunk_center_x.abs() <= TERRAIN_BOUNDARY && chunk_center_z.abs() <= TERRAIN_BOUNDARY
            })
            .collect();

        // Convert to HashMap for set operations (Bug_Game pattern)
        let new_chunk_set: HashMap<ChunkKey, ()> =
            filtered_chunks.into_iter().map(|key| (key, ())).collect();
        ...
        debug!(
            "Terrain chunks - Active: {}, Kept: {}, New: {}, Removed: {}",
...
pub fn generate_terrain_chunk_v2(
    ...
    let resolution =
        calculate_resolution_for_chunk_size(chunk_key.size, settings.min_cell_resolution);
    ...
    for z in 0..=resolution {
        for x in 0..=resolution {
            let normal = calculate_normal_v2(
                &height_data,
                x as usize,
                z as usize,
                resolution as usize,
    ...
    debug!(
        "Generated mesh with {} vertices, {} indices for chunk ({}, {})",
...
fn generate_height_v2(x: f32, z: f32, settings: &TerrainSettings, noise: &Perlin) -> f32 {
    ...
    let detail_noise =
        noise.get([x as f64 * scale as f64 * 3.0, z as f64 * scale as f64 * 3.0]) as f32;
...
fn calculate_normal_v2(
    ...
    let get_height = |x: i32, z: i32| -> f32 {
        if x < 0 || z < 0 || x > resolution as i32 || z > resolution as i32 {
            height_data[z.max(0).min(resolution as i32) as usize]
                [x.max(0).min(resolution as i32) as usize]
        } else {
...
fn calculate_terrain_color_v2(
    ...
    let texture_variation =
        large_noise * 0.1 + medium_noise * 0.08 + detail_noise * 0.06 + micro_detail * 0.03;
    ...
    let mixed_color = Color::srgb(
        base_color.to_srgba().red * (1.0 - rock_factor * 0.4)
            + dirt_color.to_srgba().red * rock_factor * 0.4,
        base_color.to_srgba().green * (1.0 - rock_factor * 0.4)
            + dirt_color.to_srgba().green * rock_factor * 0.4,
        base_color.to_srgba().blue * (1.0 - rock_factor * 0.4)
            + dirt_color.to_srgba().blue * rock_factor * 0.4,
    );
    ...
    for y in 0..texture_size {
        for x in 0..texture_size {
            ...
            let (r, g, b) = if value < 0.3 {
                ...
                (
                    0.2 * grass_intensity + 0.1,
                    0.4 + grass_intensity * 0.3,
                    0.15 + grass_intensity * 0.2,
                )
            } else {
                // Dirt/earth - browns
                let dirt_intensity = ((value - 0.6) / 0.4) as f32;
                (
                    0.4 + dirt_intensity * 0.3,
                    0.25 + dirt_intensity * 0.2,
                    0.1 + dirt_intensity * 0.1,
                )
            };
...
pub fn start_chunk_generation_tasks(
    ...
    let tasks_to_start =
        terrain_manager.max_concurrent_tasks - terrain_manager.generating_tasks.len();
    ...
    for _ in 0..tasks_to_start {
        if let Some(chunk_key) = terrain_manager.generation_queue.pop_front() {
            ...
            debug!(
                "Starting async generation for terrain chunk at ({}, {}) size {}",
...
pub fn collect_generated_chunks(
    ...
    let task_data: Vec<(ChunkKey, Entity)> = terrain_manager
        .generating_tasks
        .iter()
        .map(|(k, e)| (*k, *e))
        .collect();
    ...
    for (chunk_key, task_entity) in task_data {
        if let Ok(mut task) = generation_tasks.get_mut(task_entity) {
            ...
            if task.task.is_finished() {
                ...
                terrain_manager
                    .chunks
                    .insert(result.chunk_key, chunk_entity);
                ...
                if elapsed > 10.0 {
                    // 10 second timeout
                    warn!(
                        "Terrain generation task timed out for chunk ({}, {}) size {}",
                        chunk_key.x, chunk_key.z, chunk_key.size
                    );
                    completed_tasks.push((chunk_key, task_entity));
                }
...
pub fn process_sync_chunk_generation(
    ...
    if let Some(chunk_key) = terrain_manager.generation_queue.pop_front() {
        debug!(
            "Generating terrain chunk synchronously at ({}, {}) size {}",
            chunk_key.x, chunk_key.z, chunk_key.size
        );
