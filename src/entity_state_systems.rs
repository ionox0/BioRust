...
pub fn sync_animation_with_entity_state(
    animation_controllers: Query<(Entity, &crate::animation_systems::UnitAnimationController)>,
    entity_states: Query<&EntityState>,
    mut animation_events: EventWriter<crate::animation_systems::AnimationStateChangeEvent>,
) {
    use crate::animation_systems::AnimationState;
    ...
    for (entity, controller) in animation_controllers.iter() {
        if let Ok(entity_state) = entity_states.get(entity) {
            ...
            if controller.current_state != desired_animation {
                animation_events.send(crate::animation_systems::AnimationStateChangeEvent {
                    entity,
